"""
Management command to load ISO 27001 default controls into the system.

Usage: python manage.py load_iso27001_controls
"""

from django.core.management.base import BaseCommand
from django.db import transaction
from ismsapp.models import Controle, CategoriaControle


class Command(BaseCommand):
    help = 'Load ISO 27001 standard controls into the database'
    ...
    def handle(self, *args, **options):
        """Load ISO 27001 controls into the database."""
        self.stdout.write('Carregando controles ISO 27001...')

        created_count = 0
        updated_count = 0

        for controle_data in self.CONTROLES_ISO27001:
            nome = controle_data['nome']
            descricao = controle_data['descricao']
            categorias = controle_data['categorias']
            ...
            if created:
                ...
                if controle.descricao != descricao:
                    controle.descricao = descricao
                    controle.save()
                    updated_count += 1

            # Add categories to the control
            for categoria_tipo in categorias:
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f'Categoria "{categoria_tipo}" não encontrada para controle "{nome}"'
                        )
                    )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                f'  - {created_count} controles criados\n'
                f'  - {updated_count} controles atualizados'
            )
        )
